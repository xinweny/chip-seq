...
def main():
    ...
    for i, gsm in enumerate(gsms):
        ...
        if len(gsm_srrs) == 1:
            ...
            if is_paired(metadata_df, 'Run', srr):
                for i in [1, 2]:
                    single_fqs = ' '.join(f"fastq/{gsm_srrs}_{i}.{ext}")
                    os.system(f"cat {single_fqs} > fastq/'{gsm}_{name}_{i}.{ext}' && rm {single_fqs}")
            else:
                srr_fqs = ' '.join(f"fastq/{gsm_srrs}.{ext}")
                os.system(f"cat {srr_fqs} > 'fastq/{gsm}_{name}.{ext}' && rm {srr_fqs}")
