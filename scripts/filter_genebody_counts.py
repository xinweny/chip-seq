...
def filter_transcripts(prom_df, gb_df, rpm_thresh, dist_thresh, gene_bed, chrom_sizes, slop_l, slop_r):
    # Make counts table
    counts_df = prom_df.drop(prom_df.iloc[:, 0:7], axis=1)
    ...
    filt_gb_bt = filt_gb_bt.slop(g=chrom_sizes, s=True, l=slop_l, r=slop_r)
...
def main():
    # Parser
    parser = argparse.ArgumentParser(description='Filter gene body transcripts for ChIP-seq analysis.')
    parser.add_argument('-t', required=True, help='Path to the TSS counts file.')
    ...
    parser.add_argument('-sl', required=True, help='TSS shift.')
    parser.add_argument('-sr', required=True, help='TES shift.')
